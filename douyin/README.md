...
| `like_video` | 点赞/取消(**唯一写操作**,见下方风险提示) |
...
- 点赞是抖音风控相对宽容的写操作(比评论安全),但仍**明显**高于纯浏览的风险
- **务必只用小号**;别批量刷赞(短时间大量点赞是风控最爱抓的模式)
- 哪天点赞开始返回失败/要验证码 = 小号被风控盯上了,停手几天让它冷却
- 首次点赞相当于一次实弹测试:抖音的 digg 接口协议可能随版本微调,若返回非成功码,把 detail 发给 fable 微调参数
